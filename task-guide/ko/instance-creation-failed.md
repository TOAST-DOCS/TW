...
- 생성 요청이 거부됨: API로 인스턴스를 생성할 때 블록 스토리지 크기를 이미지의 최소 디스크 크기보다 작게 지정하면 요청이 거부되고 "잘못된 입력 수신: 블록 스토리지 크기 {0}GB는 최소 디스크 이미지 크기 {1}GB보다 작을 수 없습니다." 메시지가 표시됩니다.
- 오류 상태가 됨: 이미지에 `os_type` 속성이 없으면 인스턴스를 배치할 위치를 판단할 수 없어, 실제 자원이 충분한데도 "선택한 인스턴스 타입의 가용량이 부족하여 인스턴스를 생성할 수 없습니다." 메시지가 표시됩니다. 2024년 2월 이전에 만든 개인 이미지를 사용하는 경우 발생할 수 있습니다.
...
- **GPU·대형 타입은 사전에 자원 문의**: 이 타입들은 리전 준비와 사전 협의가 필요하므로, 필요한 시점보다 미리 제공 가능 여부를 문의합니다.
