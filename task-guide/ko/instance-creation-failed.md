...
핵심은 이미지를 만들 때 `os_type`을 지정하는 것입니다. `os_type`은 이미지가 어떤 운영체제인지 인스턴스 생성 단계에 알려 주는 값으로, 이 값이 없으면 인스턴스를 배치할 위치를 판단할 수 없어 "가용량 부족" 메시지로 거부됩니다. 자세한 필드는 [이미지 생성 API 가이드](https://docs.nhncloud.com/ko/Compute/Image/ko/public-api/#_8)를 참고하세요.
