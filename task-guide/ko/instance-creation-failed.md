...
개인 이미지로 인스턴스를 생성할 때는 이미지에 설정된 속성과 생성 설정이 맞아야 합니다. 다른 클라우드나 다른 환경에서 만든 이미지를 가져와 등록한 경우에 주로 나타나며, 다음 세 가지가 대표적입니다.

- 이미지 목록에 보이지 않음: 개인 이미지에 세부 속성이 채워져 있지 않으면 인스턴스 생성 화면의 이미지 목록에 노출되지 않을 수 있습니다.
- 생성 요청이 거부됨: 지정한 블록 스토리지가 이미지의 최소 디스크 크기보다 작으면 "잘못된 입력 수신: 블록 스토리지 크기 {0}GB는 최소 디스크 이미지 크기 {1}GB보다 작을 수 없습니다." 메시지가 표시됩니다.
- 오류 상태가 됨: 이미지에 `os_type` 속성이 없으면 인스턴스를 배치할 위치를 판단할 수 없어, 실제 자원이 충분한데도 "선택한 인스턴스 타입의 가용량이 부족하여 인스턴스를 생성할 수 없습니다." 메시지가 표시됩니다. 현재 이미지 생성 API는 `os_type`을 필수로 요구하므로 이 속성이 빠진 이미지를 새로 만들 수는 없고, 필수 요구 이전에 만들어 둔 개인 이미지를 계속 사용하는 경우에 해당합니다.

**해결 방법**

1. 이미지가 목록에 보이지 않으면 **Compute > Image**에서 해당 이미지를 선택해 **이미지 수정**으로 최소 블록 스토리지, 최소 메모리 등 세부 속성이 채워져 있는지 확인하고 값을 지정합니다.
2. 인스턴스 생성 화면 이미지 목록의 **최소 블록 스토리지(GB)** 컬럼에서 이미지가 요구하는 크기를 확인하고, 블록 스토리지 크기를 그 값 이상으로 지정해 다시 생성합니다.
3. 예전에 만든 개인 이미지라면 `os_type`을 지정해 이미지를 다시 만듭니다. `os_type`은 콘솔과 API 모두 사후에 수정할 수 없으므로 재생성이 필요합니다. 이미지 생성 API는 빈 이미지를 만드는 단계와 파일을 업로드하는 단계로 나뉘며, 첫 단계에서 `os_type`(`linux` 또는 `windows`)을 지정합니다. 자세한 필드는 [이미지 생성 API 가이드](https://docs.nhncloud.com/ko/Compute/Image/ko/public-api/#_8)를 참고하세요.
...
4. 특정 리전과 AZ에 특정 타입을 반드시 생성해야 하면, 문의하기로 생성 가능한 구성을 확인합니다.
...
3. GPU 인스턴스나 대형 타입이 필요하면 문의하기로 해당 타입의 제공 가능 여부를 확인합니다. 이 타입들은 사전 자원 협의가 필요합니다.
