...
- 생성 요청이 거부됨: API로 인스턴스를 생성할 때 블록 스토리지 크기를 이미지의 최소 디스크 크기보다 작게 지정하면 요청이 거부되고 "블록 스토리지 크기 {0}GB는 최소 디스크 이미지 크기 {1}GB보다 작을 수 없습니다." 메시지가 표시됩니다.
- 오류 상태가 됨: 이미지에 `os_type` 속성이 없으면 인스턴스를 배치할 위치를 판단할 수 없어, 실제 자원이 충분한데도 "선택한 인스턴스 타입의 가용량이 부족하여 인스턴스를 생성할 수 없습니다." 메시지가 표시됩니다. 2024년도 이전 생성한 개인 이미지를 사용하는 경우 발생할 수 있습니다.
...
3. 예전에 만든 개인 이미지라면 `os_type`을 지정해 이미지를 다시 만듭니다. 자세한 내용은 [이미지 생성 API 가이드](https://docs.nhncloud.com/ko/Compute/Image/ko/public-api/#_8)를 참고하세요.
...
### 원인 4: 사전 협의가 필요한 GPU·대형 타입

GPU 인스턴스와 대형 인스턴스 타입은 사전에 자원 할당이 협의된 뒤에 생성할 수 있습니다. GPU 인스턴스는 일반 **Instance**의 인스턴스 타입 목록에는 나타나지 않고 별도의 **GPU Instance** 서비스로 제공됩니다. 쿼터가 여유 있어도 해당 타입이 리전에 준비되어 있지 않으면 생성에 실패하며, 협의된 것과 다른 타입을 선택했을 때도 실패합니다. 공공(GOV)이나 일부 특수 리전에서 GPU·대형 타입을 요청할 때 주로 발생할 수 있습니다.

**해결 방법**

1. GPU 인스턴스는 일반 **Instance**가 아니라 **GPU Instance** 서비스에서 생성합니다. 대형 타입은 **인스턴스 타입 선택** 목록에서 필요한 사양이 제공되는지 먼저 확인합니다. 목록에 없는 타입이라면 사전 협의가 필요합니다.
2. 사전에 협의한 타입이 있다면 그 타입 이름과 정확히 일치하는지 확인해 생성합니다.
3. GPU 인스턴스나 대형 타입이 필요하면 문의하기로 해당 타입의 제공 가능 여부를 확인합니다.
