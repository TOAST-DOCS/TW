...
2. 프리플라이트 요청으로 CORS 설정을 확인하세요. 브라우저는 다른 출처로 요청을 보내기 전에 `OPTIONS` 요청으로 허용 여부를 먼저 묻습니다. 같은 요청을 직접 보내면 설정이 적용되었는지 응답 헤더로 확인할 수 있습니다.
...
4. 상태 코드가 `200`이면 두 관문을 모두 통과한 것입니다. CORS에서 막히면 `fetch`는 상태 코드를 돌려주지 못하고 `TypeError: Failed to fetch`로 실패합니다. 자세한 실패 원인은 개발자 도구 콘솔에서 확인할 수 있습니다.
    ...
    | 권한 없음 | 접근 정책 | 역할 기반 접근 정책을 사용한다면 수행하려는 작업에 맞는 권한(`Read`/`Write`)이 선택되어 있는지 확인합니다 |
