...
> 이 문서에서 다루는 원인 중 일부(원인 1, 원인 2)는 헬스 체크뿐 아니라 로드 밸런서를 거치는 일반 서비스 트래픽에도 동일하게 영향을 줄 수 있습니다. 즉 멤버가 `INACTIVE`로 표시되지 않더라도 유사한 연결 문제가 발생할 수 있습니다.

## 빠른 확인

멤버가 `INACTIVE`로 전환되는 사례의 상당수는 다음 두 가지 확인만으로 원인이 드러납니다. 5분 안에 확인할 수 있으니 먼저 시도하세요.

1. **멤버 인스턴스가 속한 보안 그룹 확인**: 멤버 인스턴스에 `default`가 아닌 별도로 생성한 보안 그룹이 적용되어 있다면, 로드 밸런서의 트래픽을 허용하는 규칙이 그 보안 그룹에 별도로 추가되어 있는지 확인합니다. `default` 보안 그룹을 그대로 쓰는 멤버는 로드 밸런서와의 통신이 자동으로 허용되지만, 커스텀 보안 그룹을 쓰는 멤버는 규칙을 직접 추가해야 합니다.
...
> 로드 밸런서에 **IP 접근 제어 그룹**을 별도로 설정해두신 경우에는, 원인 2도 함께 확인해보시기 바랍니다. IP 접근 제어 그룹을 설정하지 않았다면(콘솔에 `-`로 표시됨) 이 원인은 해당하지 않습니다.
...
Load Balancer는 생성 시 자동으로 `default` 보안 그룹에 할당됩니다. 멤버 인스턴스도 `default` 보안 그룹을 그대로 사용한다면 로드 밸런서와의 통신이 별도 설정 없이 자동으로 허용되지만, 멤버 인스턴스에 `default`가 아닌 **별도로 생성한 보안 그룹**이 적용되어 있다면 그 보안 그룹에 로드 밸런서의 트래픽을 허용하는 규칙이 없는 한 통신이 차단됩니다.

이 경우 헬스 체크 요청뿐 아니라, 로드 밸런서를 거쳐 멤버로 전달되는 **일반 서비스 트래픽도 동일하게 차단**됩니다.

### 원인 2: 로드 밸런서의 IP 접근 제어 그룹이 멤버 IP를 차단함

로드 밸런서에는 유입되는 트래픽을 제어하는 **IP 접근 제어 그룹** 기능이 있습니다. 이 기능은 클라이언트에서 로드 밸런서로 들어오는 트래픽뿐 아니라, **멤버가 로드 밸런서로 보내는 응답 트래픽(헬스 체크 응답 포함)에도 동일하게 적용**됩니다.

IP 접근 제어 타입이 **차단**으로 설정되어 있고 그 차단 대역에 멤버 인스턴스의 IP가 포함되어 있다면, 멤버가 헬스 체크에 정상적으로 응답하더라도 그 응답 패킷이 로드 밸런서 앞단에서 차단(Drop)되어 헬스 체크가 실패한 것으로 처리되고 멤버가 `INACTIVE`로 전환됩니다.
...
### 원인 1을 해결하려면(보안 그룹에 로드 밸런서 트래픽 허용 규칙 추가)
...
4. 다음과 같이 입력합니다.
   - 방향: **수신**
   - IP 프로토콜, 포트, Ether: 멤버에서 허용해야 하는 프로토콜과 포트를 선택
   - 원격: **CIDR** 선택 후 로드 밸런서의 IP를 `/32`로 입력 (예: `192.168.0.104/32`)
5. **확인**을 클릭해 저장합니다.

> **💡 알아두기**
> 보안 관점에서는 통신을 허용해야 하는 로드 밸런서의 IP만 개별로 등록하는 **방법 2**가 더 안전합니다. 다만 연결해야 하는 로드 밸런서가 여러 개이고 규칙을 여러 건 추가해 관리하기 번거롭다면, **방법 1**(default 보안 그룹 전체 허용)이 관리 측면에서 더 편리할 수 있습니다.
>
> 위 두 방법 모두 **수신(인바운드)** 규칙만 추가하면 충분합니다. NHN Cloud 보안 그룹은 stateful 방식으로 동작하므로, 수신 규칙에 따라 한 번 연결이 허용되면 그에 대한 응답(송신) 트래픽은 별도 규칙 없이도 자동으로 전달됩니다.

규칙을 추가한 뒤 Load Balancer 콘솔에서 멤버 상태가 `ACTIVE`로 바뀌는지 확인하세요.

### 원인 2를 해결하려면(IP 접근 제어 그룹에서 멤버 IP 허용)

1. NHN Cloud 콘솔에서 **Network > Load Balancer**를 클릭하고 대상 로드 밸런서의 **상세 보기**를 선택합니다.
2. 상단의 **IP 접근 제어 그룹** 탭을 선택합니다.
3. 이 로드 밸런서에 적용된 IP 접근 제어 그룹을 목록에서 **클릭**하여 상세 화면을 엽니다.
...
>
> IP 접근 제어 그룹을 설정하지 않았다면(로드 밸런서 상세 화면의 IP 접근제어 타입이 `-`로 표시됨) 모든 트래픽이 허용되므로 이 원인에는 해당하지 않습니다.
>
...
설정을 변경한 뒤 Load Balancer 콘솔에서 멤버 상태가 `ACTIVE`로 바뀌는지 확인하세요.
...
- 로드 밸런서에 적용된 IP 접근 제어 그룹 이름 및 타입(차단/허용), 규칙 목록
- 지금까지 시도한 조치 내용
...
- **멤버 등록 전 사전 점검**: Load Balancer에 멤버를 등록하기 전에, 멤버 인스턴스에 적용된 보안 그룹이 `default`가 아니라면 로드 밸런서 트래픽을 허용하는 규칙이 미리 추가되어 있는지 확인합니다. 또한 로드 밸런서에 IP 접근 제어 그룹이 설정되어 있다면, 새로 등록할 멤버의 IP가 차단 대상에 포함되어 있지 않은지 함께 확인합니다.
...
| 보안 그룹 | 인스턴스의 송수신 트래픽을 제어하는 기능. Load Balancer는 생성 시 자동으로 `default` 보안 그룹에 할당되며, 멤버도 `default` 보안 그룹을 쓰면 별도 설정 없이 통신이 허용됩니다. 멤버가 커스텀 보안 그룹을 쓰는 경우에는 로드 밸런서 트래픽을 허용하는 규칙을 직접 추가해야 합니다. |
| IP 접근 제어 그룹 | 로드 밸런서로 유입되는 트래픽(클라이언트→로드 밸런서 및 멤버→로드 밸런서 응답 포함)을 IP 또는 CIDR 단위로 허용하거나 차단하는 기능. 설정하지 않으면 모든 트래픽이 허용됩니다. |
