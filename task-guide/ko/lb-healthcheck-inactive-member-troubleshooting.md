...
> 원인 1, 원인 2는 헬스 체크뿐 아니라 일반 서비스 트래픽에도 영향을 줍니다.

## 빠른 확인

멤버가 `INACTIVE`로 전환되는 사례의 상당수는 다음 두 가지 확인만으로 원인이 밝혀집니다. 5분 안에 확인할 수 있으니 먼저 시도합니다.

1. **멤버 인스턴스가 속한 보안 그룹 확인**: 멤버 인스턴스에 `default`가 아닌 별도로 생성한 보안 그룹이 적용되어 있다면, Load Balancer의 트래픽을 허용하는 규칙이 그 보안 그룹에 별도로 추가되어 있는지 확인합니다.
...
멤버 인스턴스에 `default`가 아닌 **별도로 생성한 보안 그룹**이 적용되어 있고, 그 보안 그룹에 Load Balancer의 트래픽을 허용하는 규칙이 없다면 통신이 차단됩니다.

이 경우 헬스 체크 요청뿐 아니라, Load Balancer를 거쳐 멤버로 전달되는 **일반 서비스 트래픽도 동일하게 차단**됩니다.

**해결 방법: 보안 그룹에 Load Balancer 트래픽 허용 규칙 추가**

먼저 멤버 인스턴스에 적용된 보안 그룹이 `default`인지 확인합니다. `default`라면 별도 조치가 필요 없습니다. `default`가 아닌 커스텀 보안 그룹이라면, 아래 절차로 규칙을 추가합니다.
...
> **💡 알아두기**
> 위 규칙은 **수신(인바운드)** 규칙만 추가하면 충분합니다. NHN Cloud 보안 그룹은 stateful 방식으로 동작하므로, 수신 규칙에 따라 한 번 연결이 허용되면 그 응답(송신) 트래픽은 별도 규칙 없이도 자동으로 전달됩니다.
...
4. 재시작 후 포트가 정상적으로 열려 있는지 다시 확인합니다. `<포트번호>`에는 Load Balancer 멤버 그룹의 **멤버** 탭에 등록된 멤버 포트 값을 그대로 사용합니다.
...
4. 다음 항목을 실제 서비스 엔드포인트와 하나씩 비교합니다.
   ...
   | 상태 확인 포트 | 서비스가 실제로 리스닝하는 포트와 같은지. "멤버 포트"를 따르도록 설정되어 있다면, 멤버 등록 포트 자체가 올바른지 원인 6도 함께 확인 |
...
4. 응답 시간이 계속 길다면 애플리케이션 자체의 성능 개선을 개선해야 합니다. Cloud Monitoring에서 멤버 인스턴스의 CPU, 메모리 사용량을 확인해 리소스 병목이 있는지 점검합니다.

> **⚠️ 주의**
> 타임아웃을 지나치게 크게 설정하면 실제로 장애가 발생한 멤버를 늦게 감지합니다. 측정한 응답 시간의 2~3배 정도로 타임아웃을 설정합니다.
...
> **⚠️ 주의**
> 멤버를 삭제하는 동안 해당 멤버가 처리하던 트래픽은 나머지 멤버로 분산됩니다. 멤버가 1개뿐인 구성에서는 삭제와 재등록 사이에 서비스가 중단될 수 있습니다. 트래픽이 적은 시간대에 작업하거나, 재등록 전에 임시 멤버를 추가해 둡니다.
...
위 원인 1~6을 모두 확인해도 해결되지 않는다면 플랫폼 측 이슈일 수 있습니다. 추가로 원인을 찾기보다 NHN Cloud 고객지원에 바로 문의하세요. 문의 시 다음 정보를 함께 전달하면 진단 시간을 줄일 수 있습니다.
...
- 멤버 인스턴스의 서비스 프로세스 상태 및 리소스(CPU, 메모리) 사용량
...
- **멤버 등록 전 사전 점검**: Load Balancer에 멤버를 등록하기 전에, 멤버 인스턴스에서 서비스 프로세스가 정상적으로 실행 중이고 대상 포트가 열려 있는지 확인합니다. 또한 멤버 인스턴스에 적용된 보안 그룹이 `default`가 아니라면 Load Balancer 트래픽을 허용하는 규칙이 미리 추가되어 있는지 확인합니다. Load Balancer에 IP 접근 제어 그룹이 설정되어 있다면, 차단 타입인 경우 새로 등록할 멤버의 IP가 차단 대상에 포함되어 있지 않은지, 허용 타입인 경우 클라이언트 IP 대역뿐 아니라 멤버 인스턴스의 IP도 허용 목록에 포함되어 있는지 함께 확인합니다.
