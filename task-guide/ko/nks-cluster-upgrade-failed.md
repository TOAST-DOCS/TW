...
- [x] NHN Kubernetes Service(NKS) 서비스명 및 카테고리: `Container > NHN Kubernetes Service(NKS)` ← Playwriter 확인 완료
- [x] NKS 업그레이드가 한 번에 한 마이너 버전만 가능: v1.32.3 → v1.33.4만 선택 가능 확인 ← Playwriter 확인 완료
- [x] NKS 업그레이드 모달 제목: "클러스터 업그레이드 (마스터 구성 요소)" ← Playwriter 확인 완료
- [x] NKS 콘솔에서 노드 그룹 단위 업그레이드: 노드 그룹 > 기본 정보 > 버전 옆 **업그레이드** 버튼 ← Playwriter 확인 완료
- [x] 쿼터 관리 메뉴: 프로젝트 대시보드 > **쿼터 관리** 탭 ← Playwriter 확인 완료
- [x] 클러스터 UUID 위치: 기본 정보 탭 > 클러스터 이름 아래 ← Playwriter 확인 완료
- [x] 이벤트 메뉴: 클러스터 선택 후 하단 **이벤트** 탭 + **K8S 이벤트** 탭 존재 ← Playwriter 확인 완료
- [x] 컨트롤 플레인 로그 전송 설정 항목 존재 (기본: 사용 안 함) ← Playwriter 확인 완료
- [ ] NKS 콘솔에서 업그레이드 실패 시 작업 상태 칼럼에 표시되는 정확한 오류 상태명
- [ ] NKS 업그레이드 순서(컨트롤 플레인 → 워커 노드)가 실제와 일치하는지
- [ ] NKS에서 deprecated API 사전 검사 기능을 제공하는지
- [ ] NKS 업그레이드 시 서지(surge) 방식으로 추가 노드를 생성하는지
- [ ] NKS 업그레이드 시 애드온 비호환이 실제로 업그레이드를 차단하는지
- [ ] NKS 클러스터에서 Helm CLI 사용 가능 여부
- [ ] API 서버 로그(`/metrics` 엔드포인트) 접근 가능 여부
- [ ] deprecated API 버전별 제거 표가 NKS 지원 버전 범위에 맞는지
- [ ] 문제 예방하기 섹션의 `/metrics` 명령이 NKS에서 동작하는지
- [ ] 원인 진단 흐름 다이어그램 작성 후 Mermaid 코드 블록과 청사진 안내 어드모니션 삭제
