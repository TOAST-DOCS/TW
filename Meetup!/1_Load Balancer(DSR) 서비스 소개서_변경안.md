...
세션 분산은 5-tuple 기반이 기본이고, 세션 지속성을 켜면 소스 IP 기반으로 바뀐다. 로그인 세션처럼 동일 서버에 고정이 필요한 서비스라면 이 옵션을 선택하면 된다. 운영 중에도 변경할 수 있고, 기존 연결에는 영향을 주지 않는다.
...
커널 앞단에서 패킷을 처리하기로 선택하면서 익숙하던 자유도를 내려놓아야 했다. 동적 자료구조를 쓸 수 없고, 코드를 커널에 올릴 때 모든 실행 경로의 안전성을 정적으로 증명해야 한다. 그리고 빠뜨린 경계 검사 하나에 로드가 거부되었다. 거부 사유 로그가 직관적이지 않아 원인을 찾는 데 시간이 걸렸다.
