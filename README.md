...
기준은 **이후에 변경 사유 확인이 필요한가**입니다.

리뷰가 필요 없다고 판단하면 **PR을 만들고 바로 머지해도 됩니다.** 승인을 받는 것이 아니라 기록을 남기는 것이 목적입니다.

| PR로 합니다 | master에 바로 커밋해도 됩니다 |
| --- | --- |
| 다른 사람의 파일을 고치거나 지울 때 | 내가 쓴 문서의 오타·문구 수정 |
| 파일을 지우거나 옮기거나 이름을 바꿀 때 | 진행 중인 초안에 내용 추가 |
| 규칙이나 정책을 바꿀 때 | CHANGELOG에 한 줄 추가 |
| 다른 사람의 검토가 필요한 문서 | |
