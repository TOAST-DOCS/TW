...
기준은 **나중에 왜 그랬는지 물어볼 사람이 있는가**입니다.
...
지우고 옮기는 일을 PR로 남기는 이유는, **저장소를 봐서는 그 파일이 어디로 갔는지 알 수 없기 때문**입니다. 커밋 메시지에도 남지만 PR은 링크로 공유되고 검색이 쉽습니다. 실제로 지워진 문서를 다시 찾느라 히스토리를 뒤져야 했던 일이 있었습니다.

리뷰가 필요 없다고 판단하면 **PR을 만들고 바로 머지해도 됩니다.** 승인을 받는 것이 아니라 기록을 남기는 것이 목적입니다.

브랜치 보호로 막아두지 않았으므로 판단은 각자에게 맡깁니다.
