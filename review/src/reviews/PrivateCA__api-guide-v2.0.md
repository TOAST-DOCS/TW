...
## 변경해야 할 항목
- [기존] | 43행 | `User Access Key 토큰](...)를 참고하세요` (받침 있는 `토큰` 뒤 조사 `를`) | 조사 `를`을 `을`로 (`...token/)을 참고하세요`)
...
- [참고] | 1012행/1170행 | `"maxTTL": "31536000"`(String)와 1170행 `attributes` 내 `"maxTTL":31536000`(Number) 표기 차이 | 직렬화 형식 차이로 보이나 작성자 확인 필요
