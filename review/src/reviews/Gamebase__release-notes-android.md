...
## 변경해야 할 항목
- [변경분] | 11행 | `OneStore V21` | `ONE store v21` (문서 표준 표기, 550행 등 참고)
- [참고] | 9행 | 의존성 표기 `NHN Cloud IAP SDK (1.12.0) -> NHN IAP SDK (2.1.0)` | 모듈명 정확성 작성자 확인 권장, `->` 표기를 `→`로 수정
- [참고] | 점 목록 사용한 모든 행 | 불릿 `*` 사용(룰셋은 `-` 권장) | 문서 전체가 `*` 관례, 일괄 변경은 작성자 판단
