...
각 파일은 frontmatter + `## 변경해야 할 항목`(5열 표) + `## 참고`로 구성됩니다.
...
review_state: auto       # auto(검토 대기) → reviewing(검토중) → done(검토 완료)
---
...
## 변경해야 할 항목
- [기존] | 24행 | `참고하시기 바랍니다` | `참고하세요`
- [변경분] | 7행 | 링크 텍스트 `API v2.0 가이드` | `API v3.0 가이드`
- [참고] | 142–156행 | 통신사 고객센터 표기 | UI 경로 룰 미적용(오탐 가드)
```

**항목 형식**: `- [구분] | 위치 | AS-IS | TO-BE` — 대시보드에 **구분·위치·AS-IS·→·TO-BE 5열 표**로 렌더됩니다.
- **구분**: `[기존]`(원래 있던 이슈) / `[변경분]`(이번 배포가 바꾼 것) / `[참고]`(메모·오탐 가드, 수정 대상 아님)
- **위치**: 행번호·표·섹션 등(예: `56행`, `15행 표`). 없으면 비움.
- **AS-IS / TO-BE**: 현재 상태 / 권고하는 수정. `[참고]`는 TO-BE에 미적용 사유를 적습니다.
- 백틱 `코드`는 그대로 쓰면 대시보드에서 코드로 표기됩니다.

검토하며 할 수 있는 작업:
- **과적용/오탐 삭제**: 항목 줄을 삭제하거나 `[참고]` 태그로 강등
- **내용 보완·수정**: 위치/AS-IS/TO-BE 텍스트 수정
- **등급 조정**: `grade_change`/`grade_full`을 `G|O|R`로 수정
- **상태 갱신**: `review_state`를 `reviewing`(검토중) 또는 `done`(검토 완료)으로 변경 → 대시보드 배지 반영
- 자세한 배경·근거는 `## 참고` 섹션에 평문으로.

> 참고: TW는 검수 결과의 **타당성을 검토**합니다. 실제 가이드 원본 수정은 이후 개발자가 수행하므로,
> 항목별 '반영 완료' 체크는 없습니다(`review_state`로 검토 진행만 표시).
